...
export async function POST(request) {
  try {
    ...
    if (fs.existsSync(UPLOADS_META_PATH)) {
      allUploads = JSON.parse(fs.readFileSync(UPLOADS_META_PATH, 'utf-8'))
    }

    allUploads.unshift(uploadMeta) // הוסף בהתחלה (האחרונים ראשונים)
    fs.writeFileSync(UPLOADS_META_PATH, JSON.stringify(allUploads, null, 2))
...
export async function GET(request) {
  try {
    ...
    let allUploads = JSON.parse(fs.readFileSync(UPLOADS_META_PATH, 'utf-8'))
